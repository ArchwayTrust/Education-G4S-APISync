...
{
    [JsonObject]
    public class GETGeneralAttributes : IEndPoint<AttributeDTO>, IDisposable
    {
        const string _endPoint = "/customer/v1/academic-years/{academicYear}/students/attributes";
        ...
        public GETGeneralAttributes(RestClient client, G4SContext context, string connectionString)
        {
            _context = context;
        ...
        public async Task<bool> UpdateDatabase(string APIKey, string AcYear, string AcademyCode, int? LowestYear = null, int? HighestYear = null, int? ReportId = null, DateTime? Date = null)
        {
            try
            {
                APIRequest<GETGeneralAttributes, AttributeDTO> getAttributes = new(_client,_endPoint, APIKey, AcYear);
                ...
                foreach (var attributeDTO in attributesDTO)
                {
                    ...
                    if (attributeDTO.AttributeValues != null)
                    {
                        foreach (var attValue in attributeDTO.AttributeValues)
                        {
                            ...
                            rowAttribVal["AttributeTypeId"] = AcademyCode + AcYear + "-General-" + attributeDTO.AttributeId.ToString();
