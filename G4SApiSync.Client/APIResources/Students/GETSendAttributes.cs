...
{
    [JsonObject]
    public class GETSendAttributes : IEndPoint<AttributeDTO>, IDisposable
    {
        const string _endPoint = "/customer/v1/academic-years/{academicYear}/students/attributes/send";
        ...
        public GETSendAttributes(RestClient client, G4SContext context, string connectionString)
        {
            _context = context;
        ...
        public async Task<bool> UpdateDatabase(string APIKey, string AcYear, string AcademyCode, int? LowestYear = null, int? HighestYear = null, int? ReportId = null, DateTime? Date = null)
        {
            try
            {
                var getAttributes = new APIRequest<GETSendAttributes, AttributeDTO>(_client, _endPoint, APIKey, AcYear);
                ...
                foreach (var attributeDTO in attributesDTO)
                {
                    ...
                    if (attributeDTO.AttributeValues != null)
                    {
                        foreach (var attValue in attributeDTO.AttributeValues)
                        {
                            ...
                            rowAttribVal["AttributeTypeId"] = AcademyCode + AcYear + "-Send-" + attributeDTO.AttributeId.ToString();
                    ...
                    rowAttribType["AttributeTypeId"] = AcademyCode + AcYear + "-Send-" + attributeDTO.AttributeId.ToString();
                    rowAttribType["G4SAttributeId"] = attributeDTO.AttributeId;
                    rowAttribType["DataSet"] = AcYear;
                    rowAttribType["Academy"] = AcademyCode;
                    rowAttribType["AttributeGroup"] = "Send";
                ...
                var currentAttributes = _context.AttributeTypes
                                                .Where(i => i.DataSet == AcYear && i.Academy == AcademyCode && i.AttributeGroup == "Send");
